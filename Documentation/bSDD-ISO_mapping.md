...
| クラスプロパティ：シンボル | Property：指定されたプロパティグループ内のプロパティのシンボル | xtdProperty: シンボル | _ISOではシンボル、サブジェクト bSDDではテキスト属性。_ |
| クラスプロパティ：単位 | ✖️ | ✖️ | _ClassPropは単数形、Propertyは複数形。 説明は「単位」を参照。_ |
| クラス関係 | GroupOfProperties: 相互接続されたデータ辞書のプロパティ識別子のグループの関係。 | ✖️ | _リレーションで解くbSDDでは_ |
...
| ✖️ | ✖️ | xtd件名: フィルター | _bSDDでは、リレーションは同じような目的を果たすが、使いやすさのためにFilterの概念は実装されていない。_ |
